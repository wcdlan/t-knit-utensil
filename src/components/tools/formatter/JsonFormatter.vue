...
<template>
  <div class="space-y-4">
    ...
    <div class="flex flex-wrap gap-2">
      ...
      <button class="px-4 py-2 bg-gray-600 text-white rounded-lg text-sm font-medium hover:bg-gray-700 transition cursor-pointer"
              @click="compress">
        压缩
      </button>
      <button class="px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-600 transition cursor-pointer"
              @click="validate">
        校验
      </button>
    </div>
    <div v-if="error" :class="error.startsWith('✓') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'"
    ...
    <div v-if="output && !error" class="relative">
      <textarea
          ...
          rows="10"
      ></textarea>
      <button class="absolute top-2 right-2 px-3 py-1 bg-white border border-gray-200 rounded text-xs hover:bg-gray-50 transition cursor-pointer"
              @click="copy">
        复制
      </button>
    </div>
