...
<template>
	<!-- 快捷链接：站点配置的右上角图标链接组（兼容内置 Iconify 图标与自定义上传图片） -->
	<div v-if="quickLinks.length" class="absolute right-4 top-4 z-10 flex items-center gap-1.5">
		<n-tooltip v-for="link in quickLinks" :key="link.url">
			<template #trigger>
				<a
					:href="link.url"
