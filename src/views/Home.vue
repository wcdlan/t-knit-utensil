	...
	<div>
		...
		<div v-for="group in filteredGroups" :key="group.id" class="mb-5">
			<div class="flex items-center gap-1.5 mb-2 cursor-pointer select-none" @click="toggle(group.id)">
				<span :class="{ 'rotate-90': !collapsed[group.id] }" class="text-[10px] text-gray-400 transition-transform"
					>&#9654;</span
				>
			...
			<div v-show="!collapsed[group.id]" class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-1.5">
				<router-link
