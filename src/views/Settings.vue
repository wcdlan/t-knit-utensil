...
<template>
  <div class="max-w-2xl mx-auto">
    ...
    <div
      v-if="!isDev"
      class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-700 mb-6"
    >
    ...
    <div class="mt-6 p-4 bg-gray-50 rounded-lg text-xs text-gray-500 leading-relaxed">
      运行时数据文件：<code class="bg-gray-100 px-1 rounded">site.db.json</code>（node-json-db
      管理）。 默认配置文件：<code class="bg-gray-100 px-1 rounded">site.config.json</code
      >（仅作初始默认值）。
    </div>
  </div>
</template>
