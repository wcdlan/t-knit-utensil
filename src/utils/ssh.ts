...
function rsaJwkToSsh(nB64u: string, eB64u: string, comment?: string): string {
	...
	const body = concatArrays(sshString(new TextEncoder().encode('ssh-rsa')), sshMpint(e), sshMpint(n));
...
async function encryptPrivateKey(pkcs8Der: ArrayBuffer, passphrase: string): Promise<ArrayBuffer> {
	...
	// Encrypt PKCS#8 DER
	const encrypted = await crypto.subtle.encrypt({ name: 'AES-CBC', iv }, aesKey, pkcs8Der);
	...
	const epki = derSequence(concatArrays(algoId, derOctetString(new Uint8Array(encrypted))));
...
function buildPbes2AlgoId(salt: Uint8Array, iterations: number, iv: Uint8Array): Uint8Array {
	...
	const pbkdf2Params = derSequence(
		concatArrays(
			...
			derSequence(concatArrays(encodeOidFull(HMAC_SHA256_OID), derNull()))
		)
	);

	// PBKDF2 AlgorithmIdentifier
	const pbkdf2AlgoId = derSequence(concatArrays(encodeOidFull(PBKDF2_OID), pbkdf2Params));

	// AES-256-CBC AlgorithmIdentifier with IV parameter
	const aesAlgoId = derSequence(concatArrays(encodeOidFull(AES256_CBC_OID), derOctetString(iv)));

	// PBES2-params
	const pbes2Params = derSequence(concatArrays(pbkdf2AlgoId, aesAlgoId));

	// PBES2 AlgorithmIdentifier
	return derSequence(concatArrays(encodeOidFull(PBES2_OID), pbes2Params));
}
